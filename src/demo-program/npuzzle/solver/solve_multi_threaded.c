...
static void* generator_worker(void* thread_params){
	...
	thread_params_t* parameters = (thread_params_t*)thread_params;
 ...
 */
void print_solution_path(state_t* solution_path, const int N, int pathlen, int num_unique_configs, double time_spent_CPU){
	...
	printf("Memory consumed: %.2f MB\n", (sizeof(state_t) + N*N*sizeof(short)) * num_unique_configs / 1048576.0);
...
state_t* solve(mempool_t* mempool, const int N, state_t* start_state, state_t* goal_state, int solver_mode){
	...
	closed_t* closed = initialize_closed();
